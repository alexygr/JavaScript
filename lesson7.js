...
var BARRIER_TIMER = 10000; // Интервал появление препятствий
...
function startGame() {
    if (!gameIsRunning) {
        ...
        barrier_timer = setInterval(createBarrier, BARRIER_TIMER);//каждые 10000мс запускаем функцию createBarrier
    }
}
...
function move() {
    ...
    if (direction == 'x-') {
        ...
        new_unit = document.getElementsByClassName('cell-' + (coord_y) + '-' + (coord_x + 1))[0];
        if (new_unit == undefined) {
            new_unit = document.getElementsByClassName('cell-' + (coord_y - 1) + '-' + 0)[0];
        }
    }
    else if (direction == 'y+') {
...
// проверка на клетку с препятствием
function isBarrierUnit(unit) {
    var check = false;
    var unit_cell_classes = unit.getAttribute('class').split(' ');
    if (unit_cell_classes.includes('barrier-unit')) {
        check = true;
    }
    return check;
}
...
function createBarrier() {
    ...
    while (!barrierCreated) { // пока барьер не создан
        ...
        // проверка на змейку еду и барьер
        if ((!barrier_cell_classes.includes('snake-unit')) &&
            (!barrier_cell_classes.includes('barrier-unit')) &&
